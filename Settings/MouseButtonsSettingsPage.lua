...
local page, B, D, list, stateIndicator
...
function BC.Refresh(assignedSlot)
    ...
    for index, slotId in ipairs(order) do
        ...
        AC.SetActionRowState(row, {
            ...
            weaponSetName = entry and B.GetSlotWeaponSet
                and B.GetSlotWeaponSet(layoutKey, slotId),
...
function BC.Create(parent, deps)
    ...
    list = AC.CreateActionList(page, "ApogeePartyHealthBarsMouseButtonsSettingsPageScroll")
    ...
    for _, slotId in ipairs(B.GetDisplayOrder()) do
        ...
        if row.weapon then
            row.weapon:SetSelectionCallback(function(setName)
                local ok, message = B.SetSlotWeaponSet(
