...
ApogeePartyHealthBars_KeyboardSettingsPage = {}
local KC = ApogeePartyHealthBars_KeyboardSettingsPage

local page, K, D, list, stateIndicator
...
function KC.Create(parent, deps)
    D, K = deps, deps.KeyboardActions
    ...
    list = AC.CreateActionList(page, "ApogeePartyHealthBarsKeyboardSettingsPageScroll")
    ...
    for _, slotId in ipairs(K.GetDisplayOrder()) do
        ...
        row:SetScript("OnClick", function()
            ...
            if (cursorType == "spell" or cursorType == "item") and D.AssignCursorDrop then
                D.AssignCursorDrop("keyboard", boundSlotId, selectedLayout())
            end
        end)
