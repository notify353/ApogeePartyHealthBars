...
ApogeePartyHealthBars_ProfilesSettingsPage = {}
local P = ApogeePartyHealthBars_ProfilesSettingsPage

local page, D, form, profileDropdown, copyDropdown, statusText
...
local function updateControls()
    ...
    if shareSection and shareSection.label then
        shareSection.label:SetText(sharingSupported and "Share" or "Share — Unavailable")
        shareSection.label:SetTextColor(sharingSupported and 0.82 or 1,
            sharingSupported and 0.74 or 0.48, sharingSupported and 0.48 or 0.36)
    end
...
function P.Refresh()
    if not page or not D.ProfileStore.GetActiveProfile() then return end
...
local function createEditBox(parent, width, height)
    ...
    local inset = frame:CreateTexture(nil, "BACKGROUND", nil, -1)
    inset:SetPoint("TOPLEFT", frame, "TOPLEFT", 2, -2)
    inset:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -2, 2)
    inset:SetColorTexture(0.025, 0.025, 0.035, 0.96)
    frame.apogeeInset = inset
    local edit = frame.EditBox
    edit:SetWidth(width - 22)
    edit:SetFontObject("ChatFontNormal")
    edit:SetTextColor(0.72, 0.74, 0.78)
...
function P.Create(parent, deps)
    ...
    form = UIH.CreateFormScaffold(page, "ApogeePartyHealthBarsProfilesSettingsPageScroll",
    ...
    useButton = UIH.CreateButton(profileRow, "Activate Profile", 120, 22, "primary")
    ...
    newButton = UIH.CreateButton(actionsRow, "Create", actionWidth, 22, "primary")
    ...
    deleteButton = UIH.CreateButton(actionsRow, "Delete", actionWidth, 22, "danger")
    ...
    copyButton = UIH.CreateButton(copyRow, "Replace Active", 126, 22, "danger")
    ...
    importButton = UIH.CreateButton(shareRow, "Import", shareWidth, 22, "primary")
    ...
    nameFrame = CreateFrame("Frame", nil, page, "BackdropTemplate")
    nameFrame:SetPoint("TOPLEFT", page, "TOPLEFT", 36, -70)
    nameFrame:SetSize(C.CONFIG_CONTENT_W - 72, 116); nameFrame:SetFrameLevel(page:GetFrameLevel() + 20)
    ...
    nameSave = UIH.CreateButton(nameFrame, "Save", 76, 22, "primary")
    ...
    shareFrame = CreateFrame("Frame", nil, page, "BackdropTemplate")
    shareFrame:SetAllPoints(page); shareFrame:SetFrameLevel(page:GetFrameLevel() + 10)
    ...
    shareTitle:SetPoint("TOPRIGHT", shareFrame, "TOPRIGHT", -10, -10)
    shareTitle:SetHeight(16)
    shareTitle:SetJustifyH("RIGHT")
    shareTitle:SetTextColor(1, 0.82, 0)
    shareTextFrame, shareText = createEditBox(shareFrame, C.CONFIG_CONTENT_W - 20, 190)
    shareTextFrame:SetPoint("TOPLEFT", shareFrame, "TOPLEFT", 10, -34)
    ...
    shareStatusFrame:SetSize(C.CONFIG_CONTENT_W - 20, 62)
    ...
    sharePrimary = UIH.CreateButton(shareFrame, "Review Import", 108, 22, "primary")
    sharePrimary:SetPoint("RIGHT", shareCancel, "LEFT", -8, 0)
    shareMerge = UIH.CreateButton(shareFrame, "Merge Selected", 96, 22, "primary")
    shareMerge:SetPoint("RIGHT", sharePrimary, "LEFT", -8, 0); shareMerge:Hide()
    shareReplace = UIH.CreateButton(shareFrame, "Replace Selected", 104, 22, "danger")
...
P.GetFrame = function() return page end
