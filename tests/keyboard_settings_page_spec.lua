...
function ApogeePartyHealthBars_ActionSettingsComponents.CreateActionList()
    actionList = {
...
function ApogeePartyHealthBars_ActionSettingsComponents.CreateActionRow()
    local row = widget()
...
function ApogeePartyHealthBars_ActionSettingsComponents.SetActionRowState(row, options)
    row.state = options
...
function ApogeePartyHealthBars_ActionSettingsComponents.LayoutActionList(list, rows, layoutControl)
    list.visibleRows, list.layoutControl = rows, layoutControl
end
function ApogeePartyHealthBars_ActionSettingsComponents.SetActionListStatus(list, message, good)
    list.status:SetText(message or ""); list.statusGood = good
end
function ApogeePartyHealthBars_ActionSettingsComponents.OpenEditor(options) editorOptions = options; return true end
function ApogeePartyHealthBars_ActionSettingsComponents.CloseEditor() closeCount = closeCount + 1; editorOptions = nil end
...
dofile("Settings/KeyboardSettingsPage.lua")
local config = ApogeePartyHealthBars_KeyboardSettingsPage
config.Create(widget(), {
    KeyboardActions = runtime,
...
assert(droppedFeature == "keyboard" and droppedSlot == "keyR" and droppedLayout == "base",
