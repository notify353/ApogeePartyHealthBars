ApogeePartyHealthBars_ConfigSurfaces = {}
...
function M.Register(key, frame, options)
    assert(type(key) == "string" and key ~= "", "ConfigSurfaces requires a surface key")
    assert(frame and not surfaces[key], "ConfigSurfaces duplicate or invalid surface: " .. key)
    options = options or {}
    local surface = {
        key = key,
        frame = frame,
