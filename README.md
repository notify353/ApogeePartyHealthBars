...
Middle-click the minimap button or use `/aphb board` to open Dungeon Board. It shows recent dungeon requests seen in chat channels you have already joined and in guild chat, highlights guild requests in their own leading section, groups other exact matches by dungeon, and keeps ambiguous or multi-dungeon posts in one separate section. It does not join channels, inspect players, or retain requests across sessions.
