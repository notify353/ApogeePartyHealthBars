...
Each grouped boss encounter has exactly one Circle primary. Keep `boss = true` on every actual boss so the Book retains `[BOSS]`; give secondary bosses Skull, Cross, or No Auto Mark when that communicates the real order. Supply explicit shared encounter metadata for every grouped boss. Assign `stagingContext` to an automatically marked boss add when its NPC type reliably belongs to that boss encounter; the value must reference the encounter's primary or shared key. Do not group reused trash types when their actual pull cannot be known from the current target. `priority` controls deterministic Book order only and never arbitrates live icon ownership. Every automatically marked entry requires a positive integer `autoMarkRank`; lower values win only against another explicitly targeted candidate for the same icon before combat. Author it from the evidence-backed kill order, and override the helper default whenever Book order and live marker precedence differ. Pre-pull staging changes context only for automatically markable targets, expires after 15 seconds without another such target, preserves observed manual owners during automatic resets, and never scans for pack membership.
