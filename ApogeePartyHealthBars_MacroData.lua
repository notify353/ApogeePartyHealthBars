...
D.SyntaxTopics = {
    { id = "syntax-targetenemy", title = "Conditional Enemy Targeting", body = "/targetenemy [noexists][dead][help]", explanation = "Finds a hostile target only when the current target is missing, dead, or friendly.", applied = "Generated auto-attack families and explicit combat recipes, not ordinary spells.", why = "It preserves a valid living enemy instead of changing targets on every press.", tradeoffs = "It is unsafe as a universal default because friendly, utility, stealth, and crowd-control spells may not want an enemy target.", copyable = false },
    { id = "syntax-startattack", title = "Start Melee Auto-Attack", body = "/startattack", explanation = "Starts melee auto-attack without toggling it off on another press.", applied = "The melee Attack template and the ranged auto-attack melee fallback.", why = "Confirmed attack actions engage reliably even when another cast cannot fire.", tradeoffs = "Do not add it universally: it can break crowd control, cancel stealth plans, or start unwanted combat.", copyable = false },
    { id = "syntax-nochanneling", title = "Spell-Specific Channel Guard", body = "/cast [nochanneling:Mind Flay] Mind Flay(Rank 7)", explanation = "Casts unless that same named spell is already channeling.", applied = "Every newly generated ordinary spell macro, using the assigned spell's localized base name.", why = "It makes channel keys spammable without preventing other abilities from interrupting the channel.", tradeoffs = "It also prevents another copy of the spell from queuing during the channel, so latency can leave a small gap afterward.", copyable = false },
    { id = "syntax-bang", title = "Prevent Toggle-Off with !", body = "/cast !Auto Shot", explanation = "The ! prefix turns on a toggle-style spell but does not turn it off on another press.", applied = "Generated Auto Shot and client-confirmed ranged auto-attacks; selected custom toggle recipes also use it.", why = "Repeated key presses cannot cancel a supported repeating or toggle action.", tradeoffs = "Classic wand Shoot has been inconsistent with ! and uses a dedicated sequence. Do not add ! indiscriminately when intentional cancellation matters.", copyable = false },
    { id = "syntax-ranked", title = "Rank-Qualified Spell Names", body = "/cast Fireball(Rank 1)", explanation = "Keeps the Spellbook subtext so the macro casts the exact assigned rank.", applied = "The cast portion of generated spell macros when the client supplies rank text.", why = "Classic characters may intentionally assign different ranks of the same spell.", tradeoffs = "The channel condition uses the unranked base name because the active channel is identified by spell name.", copyable = false },
    { id = "syntax-conditions", title = "Macro Conditions", body = "/cast [@mouseover,help,nodead] Flash Heal", explanation = "Bracketed conditions choose whether and where a secure macro command may act.", applied = "Generated templates use target-state and channel conditions; users can add modifiers or unit targeting.", why = "Conditions provide predictable choices without addon code making protected combat decisions.", tradeoffs = "Conditions cannot inspect arbitrary cooldown, resource, or rotation state.", copyable = false },
    { id = "syntax-reset", title = "Reset to Generated Default", body = "Reset", explanation = "Reset rebuilds the selected action's macro from its current spell or item.", applied = "The focused macro editor for Shortcuts, Keys, and Wheel.", why = "It restores the latest smart template without clearing the assigned action or sound.", tradeoffs = "Reset replaces the current custom macro text after the user saves it.", copyable = false },
    { id = "syntax-custom", title = "Custom Macro Preservation", body = "Custom text is preserved", explanation = "Normalization, profiles, imports, movement, and layout changes retain nonblank macro text exactly.", applied = "Every stored Shortcut, Key, and Wheel action.", why = "Generated defaults are a starting point rather than an ownership claim over user edits.", tradeoffs = "Existing assignments are not automatically upgraded; use Reset to adopt a newer template.", copyable = false },
    ...
    { id = "syntax-castsequence", title = "Castsequence State and Resets", body = "/castsequence reset=target/2 !Shoot, null", explanation = "Advances only after a successful step and resets only when its declared target, combat, or idle-time condition occurs.", applied = "The wand-specific generated template and carefully tested custom sequences.", why = "An intentionally invalid second step can absorb repeated presses that would otherwise toggle Shoot off.", tradeoffs = "Sequences cannot choose by cooldown or resources and can become stuck when a required step cannot succeed.", copyable = false },
}
