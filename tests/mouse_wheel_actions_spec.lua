...
dofile("Actions/MouseWheel/MouseWheelData.lua")
...
dofile("Actions/MouseWheel/MouseWheelLayouts.lua")
...
dofile("Actions/MouseWheel/MouseWheelActions.lua")
local data, wheel = ApogeePartyHealthBars_MouseWheelData, ApogeePartyHealthBars_MouseWheelActions
...
local ownershipBeforeSpecChange = ApogeePartyHealthBars_S.charSv.mouseWheelActions.ownership
activeSpecGroup = 2
...
assert(saveCount == bindingSavesBeforeSpecChange
    and ApogeePartyHealthBars_S.charSv.mouseWheelActions.ownership == ownershipBeforeSpecChange,
...
ApogeePartyHealthBars_S.charSv.mouseWheelActions.profiles.corrupt = {
    layouts = { missingSlots = {}, invalidSlots = { slots = "not a table" } },
}
wheel.RefreshItemInfo()
ApogeePartyHealthBars_S.charSv.mouseWheelActions.profiles.corrupt = nil
...
for _, slot in ipairs(data.SLOTS) do
    local expected = "CLICK " .. slot.buttonName .. ":LeftButton"
    ...
    assert(#button.registeredClicks == 1 and button.registeredClicks[1] == "LeftButtonUp",
        "physical Wheel target did not follow the release CVar phase")
...
assert(wheel.RefreshPhysicalClickRegistration(), "physical click phase did not refresh")
for _, slot in ipairs(data.SLOTS) do
    local button = assert(namedFrames[slot.buttonName])
    assert(#button.registeredClicks == 1 and button.registeredClicks[1] == "LeftButtonDown",
        "physical Wheel target did not follow the key-down CVar phase")
end
inCombat = true
actionButtonUseKeyDown = false
assert(not wheel.RefreshPhysicalClickRegistration(),
    "physical click phase changed during combat")
for _, slot in ipairs(data.SLOTS) do
    assert(namedFrames[slot.buttonName].registeredClicks[1] == "LeftButtonDown",
        "combat changed a protected physical click registration")
end
inCombat = false
wheel.OnCombatEnded()
for _, slot in ipairs(data.SLOTS) do
    assert(namedFrames[slot.buttonName].registeredClicks[1] == "LeftButtonUp",
...
assert(droppedFeature == "mouseWheel" and droppedSlot == "normalUp" and droppedLayout == PRIMARY,
...
assert(bindings.MOUSEWHEELUP == "CLICK ApogeePartyHealthBarsWheelNormalUp:LeftButton",
...
assert(bindings["SHIFT-MOUSEWHEELUP"]:find("CLICK ApogeePartyHealthBarsWheelShiftUp", 1, true),
...
assert(bindings.MOUSEWHEELUP:find("CLICK ApogeePartyHealthBarsWheelNormalUp", 1, true),
...
ApogeePartyHealthBars_S.charSv.mouseWheelActions = {
    schemaVersion = 3, bindingVersion = 1, ownership = {},
    profiles = { ["1"] = { layouts = { base = { slots = {} } } } },
}
for _, slot in ipairs(data.SLOTS) do
    ApogeePartyHealthBars_S.charSv.mouseWheelActions.profiles["1"].layouts.base.slots[slot.id] = { macroText = "" }
    bindings[slot.key] = (slot.id == "normalUp" and "CAMERAZOOMIN")
        or (slot.id == "normalDown" and "CAMERAZOOMOUT") or ""
end
ApogeePartyHealthBars_S.charSv.mouseWheelActions.profiles["1"].layouts.base.slots.normalUp = {
    displaySpellId = 78,
...
assert(ApogeePartyHealthBars_S.charSv.mouseWheelActions.schemaVersion == 6
...
assert(ApogeePartyHealthBars_S.charSv.mouseWheelActions.ownership["2"] == nil,
