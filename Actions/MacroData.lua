-- Immutable combat macro recipes for the supported Classic clients.
...
D.SyntaxTopics = {
    ...
    { id = "syntax-mouseover-help", title = "Friendly Mouseover Priority", body = "/cast [@mouseover,help,nodead][help,nodead][@player] Flash Heal", explanation = "Uses a living friendly mouseover first, then a friendly target, then the player.", applied = "Optional custom healing, buff, cleanse, and utility macros outside native Party Frame Clicks.", why = "The current hostile target can stay selected while the spell goes to the intended ally.", tradeoffs = "Mouse position becomes meaningful; omit the fallback branches when an accidental target would be worse than a failed cast.", copyable = false },
    { id = "syntax-focus", title = "Focus with Normal Fallback", body = "/cast [@focus,harm,nodead][] Counterspell", explanation = "Uses a living hostile focus when available and otherwise preserves the spell's normal targeting.", applied = "Optional interrupts, crowd control, dispels, and utility spells on supported clients.", why = "A priority enemy can be controlled without losing the current target.", tradeoffs = "The focus must be maintained deliberately, and some spells need friendly rather than hostile conditions.", copyable = false },
