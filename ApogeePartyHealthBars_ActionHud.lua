...
local BUTTON_GRID_LEFT = C.ROW_CONTENT_W + C.SHORTCUT_ICON_SIZE + C.SHORTCUT_ICON_GAP * 2
local BUTTON_GRID_WIDTH = C.SHORTCUT_ICON_SIZE * 3 + C.SHORTCUT_ICON_GAP * 2
local FEEDBACK_LEFT = BUTTON_GRID_LEFT + BUTTON_GRID_WIDTH + C.SHORTCUT_ICON_GAP * 2
-- The right-side placement has room for longer action names without wrapping.
...
function H.Show(source, triggerLabel, actionName, duration)
    ...
    if S and S.sv and S.sv.actionFeedbackEnabled == false then
        H.Clear()
        return false
    end
