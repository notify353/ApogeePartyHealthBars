ApogeePartyHealthBars_DungeonBoardCatalog = {}
local Catalog = ApogeePartyHealthBars_DungeonBoardCatalog

local definitions = {
    { key = "RFC", name = "Ragefire Chasm", chatLabel = "RFC", expansion = "classicEra", minLevel = 15, maxLevel = 20,
        aliases = { "rfc", "ragefire", "chasm" } },
    { key = "WC", name = "Wailing Caverns", chatLabel = "WC", expansion = "classicEra", minLevel = 17, maxLevel = 25,
        aliases = { "wc", "wailing", "caverns" } },
    { key = "DM", name = "The Deadmines", chatLabel = "VC", expansion = "classicEra", minLevel = 17, maxLevel = 26,
        aliases = { "deadmines", "vc", "vancleef", "dead mines", "mine" } },
    { key = "SFK", name = "Shadowfang Keep", chatLabel = "SFK", expansion = "classicEra", minLevel = 18, maxLevel = 26,
        aliases = { "sfk", "shadowfang" } },
    { key = "BFD", name = "Blackfathom Deeps", chatLabel = "BFD", expansion = "classicEra", minLevel = 20, maxLevel = 30,
        aliases = { "bfd", "blackfathom", "fathom" } },
    { key = "STK", name = "The Stockade", chatLabel = "Stocks", expansion = "classicEra", minLevel = 22, maxLevel = 30,
        aliases = { "stk", "stock", "stockade", "stockades" } },
    { key = "GNO", name = "Gnomeregan", chatLabel = "Gnomer", expansion = "classicEra", minLevel = 24, maxLevel = 34,
        aliases = {
            "gnomer", "gno", "gnomeregan", "gnomeragan", "gnome", "gnomregan",
            "gnomragan", "gnom", "gnomergan",
        } },
    { key = "RFK", name = "Razorfen Kraul", chatLabel = "RFK", expansion = "classicEra", minLevel = 30, maxLevel = 40,
        aliases = { "rfk", "kraul" } },
    { key = "SMG", name = "Scarlet Monastery - Graveyard", chatLabel = "GY", expansion = "classicEra",
        minLevel = 26, maxLevel = 36, aliases = { "smgy", "smg", "gy", "graveyard" } },
    { key = "SML", name = "Scarlet Monastery - Library", chatLabel = "Lib", expansion = "classicEra",
        minLevel = 29, maxLevel = 39, aliases = { "smlib", "sml", "lib", "library" } },
    { key = "SMA", name = "Scarlet Monastery - Armory", chatLabel = "Arm", expansion = "classicEra",
        minLevel = 34, maxLevel = 42,
        aliases = { "smarm", "sma", "arm", "armory", "herod", "armoury", "arms" } },
    { key = "SMC", name = "Scarlet Monastery - Cathedral", chatLabel = "Cath", expansion = "classicEra",
        minLevel = 37, maxLevel = 45, aliases = { "smcath", "smc", "cath", "cathedral" } },
    { key = "RFD", name = "Razorfen Downs", chatLabel = "RFD", expansion = "classicEra", minLevel = 40, maxLevel = 50,
        aliases = { "rfd", "downs" } },
    { key = "ULD", name = "Uldaman", chatLabel = "Ulda", expansion = "classicEra", minLevel = 37, maxLevel = 45,
        aliases = { "uld", "ulda", "uldaman", "ulduman", "uldman", "uldama", "udaman" } },
    { key = "ZF", name = "Zul'Farrak", chatLabel = "ZF", expansion = "classicEra", minLevel = 44, maxLevel = 54,
        aliases = {
            "zf", "zul farrak", "zulfarrak", "zulfarak", "zulfa", "zulf",
        } },
    { key = "MAR", name = "Maraudon", chatLabel = "Mara", expansion = "classicEra", minLevel = 32, maxLevel = 44,
        aliases = {
            "mar", "mara", "maraudon", "mauradon", "mauro", "maurodon", "princessrun",
            "maraudin", "maura", "marau", "mauraudon",
        } },
    { key = "ST", name = "The Temple of Atal'Hakkar", chatLabel = "ST", expansion = "classicEra",
        minLevel = 50, maxLevel = 60, aliases = { "st", "sunken", "atal", "temple" } },
    { key = "BRD", name = "Blackrock Depths", chatLabel = "BRD", expansion = "classicEra", minLevel = 49, maxLevel = 61,
        aliases = { "brd", "emperor", "emp", "arenarun", "angerforge", "blackrockdepth" } },
    { key = "DME", name = "Dire Maul - East", chatLabel = "DME", expansion = "classicEra", minLevel = 54, maxLevel = 60,
        aliases = { "dme", "dmeast", "east", "puzilin", "jumprun" } },
    { key = "DMW", name = "Dire Maul - West", chatLabel = "DMW", expansion = "classicEra", minLevel = 57, maxLevel = 60,
        aliases = { "dmw", "dmwest", "west" } },
    { key = "DMN", name = "Dire Maul - North", chatLabel = "DMN", expansion = "classicEra", minLevel = 58, maxLevel = 60,
        aliases = { "dmn", "dmnorth", "north", "tribute", "dmt" } },
    { key = "STR", name = "Stratholme", chatLabel = "Strat", expansion = "classicEra", minLevel = 58, maxLevel = 60,
        aliases = {
            "stratlive", "live", "living", "stratud", "undead", "ud", "baron", "stratholme",
            "stath", "stratholm", "strah", "strath", "strat", "starth",
        } },
    { key = "SCH", name = "Scholomance", chatLabel = "Scholo", expansion = "classicEra", minLevel = 58, maxLevel = 60,
        aliases = { "scholomance", "scholo", "sholo", "sholomance" } },
    { key = "LBRS", name = "Lower Blackrock Spire", chatLabel = "LBRS", expansion = "classicEra",
        minLevel = 57, maxLevel = 60, aliases = { "lower", "lbrs", "lrbs" } },
    { key = "UBRS", name = "Upper Blackrock Spire", chatLabel = "UBRS", expansion = "classicEra",
        minLevel = 58, maxLevel = 60, maxPlayers = 10,
        aliases = { "upper", "ubrs", "urbs", "rend" } },

    { key = "RAMPS", name = "Hellfire Ramparts", chatLabel = "Ramps", expansion = "tbcAnniversary",
        minLevel = 59, maxLevel = 67, aliases = { "ramparts", "rampart", "ramp", "ramps" } },
    { key = "BF", name = "The Blood Furnace", chatLabel = "BF", expansion = "tbcAnniversary",
        minLevel = 61, maxLevel = 68, aliases = { "furnace", "furn", "bf" } },
    { key = "SP", name = "The Slave Pens", chatLabel = "SP", expansion = "tbcAnniversary",
        minLevel = 62, maxLevel = 69, aliases = { "slavepens", "pens", "sp" } },
    { key = "UB", name = "The Underbog", chatLabel = "UB", expansion = "tbcAnniversary",
        minLevel = 63, maxLevel = 70, aliases = { "underbog", "ub" } },
    { key = "MT", name = "Mana-Tombs", chatLabel = "MT", expansion = "tbcAnniversary",
        minLevel = 64, maxLevel = 71, aliases = { "manatombs", "mana", "mt", "tomb", "tombs" } },
    { key = "CRYPTS", name = "Auchenai Crypts", chatLabel = "Crypts", expansion = "tbcAnniversary",
        minLevel = 65, maxLevel = 72,
        aliases = { "crypts", "crypt", "auchenai", "ac", "acrypts", "acrypt" } },
    { key = "OHB", name = "Old Hillsbrad Foothills", chatLabel = "OHB", expansion = "tbcAnniversary",
        minLevel = 66, maxLevel = 73,
        aliases = { "ohb", "oh", "ohf", "durnholde", "hillsbrad", "escape" } },
    { key = "SETH", name = "Sethekk Halls", chatLabel = "Sethekk", expansion = "tbcAnniversary",
        minLevel = 67, maxLevel = 73, aliases = { "sethekk", "seth", "sethek" } },
    { key = "SL", name = "Shadow Labyrinth", chatLabel = "SL", expansion = "tbcAnniversary",
        minLevel = 69, maxLevel = 75, aliases = { "sl", "slab", "labyrinth", "lab" } },
    { key = "SH", name = "The Shattered Halls", chatLabel = "SH", expansion = "tbcAnniversary",
        minLevel = 69, maxLevel = 75, aliases = { "sh", "shattered", "shatered", "shaterred" } },
    { key = "BM", name = "The Black Morass", chatLabel = "BM", expansion = "tbcAnniversary",
        minLevel = 69, maxLevel = 75, aliases = { "morass", "bm", "black" } },
    { key = "SV", name = "The Steamvault", chatLabel = "SV", expansion = "tbcAnniversary",
        minLevel = 69, maxLevel = 75,
        aliases = { "sv", "steamvault", "steamvaults", "steam vault", "valts" } },
    { key = "MECH", name = "The Mechanar", chatLabel = "Mech", expansion = "tbcAnniversary",
        minLevel = 70, maxLevel = 75, aliases = { "mech", "mechanar" } },
    { key = "BOT", name = "The Botanica", chatLabel = "Bot", expansion = "tbcAnniversary",
        minLevel = 70, maxLevel = 75, aliases = { "botanica", "bot" } },
    { key = "ARC", name = "The Arcatraz", chatLabel = "Arc", expansion = "tbcAnniversary",
        minLevel = 70, maxLevel = 75, aliases = { "arc", "arcatraz", "alcatraz" } },
    { key = "MGT", name = "Magisters' Terrace", chatLabel = "MgT", expansion = "tbcAnniversary",
        minLevel = 68, maxLevel = 75,
        aliases = { "mgt", "mrt", "terrace", "magisters", "magister" } },
}

local byKey = {}
for _, definition in ipairs(definitions) do
    definition.maxPlayers = definition.maxPlayers or 5
    if definition.expansion == "tbcAnniversary" then
        definition.heroicMinLevel = definition.heroicMinLevel or 70
    end
    byKey[definition.key] = definition
end

local function cloneDefinition(definition)
    if not definition then return nil end
    local copy = {}
    for key, value in pairs(definition) do
        if key == "aliases" then
            copy.aliases = {}
            for index, alias in ipairs(value) do
                copy.aliases[index] = alias
            end
        else
            copy[key] = value
        end
    end
    return copy
end

local function supportsClient(definition, clientFlavor)
    if clientFlavor == "classicEra" then
        return definition.expansion == "classicEra"
    end
    if clientFlavor == "tbcAnniversary" then
        return definition.expansion == "classicEra" or definition.expansion == "tbcAnniversary"
    end
    return false
end

function Catalog.GetDungeon(key)
    return cloneDefinition(byKey[key])
end

function Catalog.GetDungeons(clientFlavor)
    local result = {}
    for _, definition in ipairs(definitions) do
        if supportsClient(definition, clientFlavor) then
            result[#result + 1] = cloneDefinition(definition)
        end
    end
    return result
end

function Catalog.IsFivePlayer(key)
    local definition = byKey[key]
    return definition ~= nil and definition.maxPlayers == 5
end

function Catalog.IsLevelAppropriate(key, playerLevel, heroic, levelWindow)
    local definition = byKey[key]
    if not definition or type(playerLevel) ~= "number" then return false end
    if heroic then
        return definition.heroicMinLevel ~= nil
            and playerLevel >= definition.heroicMinLevel
    end
    local minLevel = levelWindow and tonumber(levelWindow.minLevel) or playerLevel
    local maxLevel = levelWindow and tonumber(levelWindow.maxLevel) or playerLevel
    if not minLevel or not maxLevel then return false end
    return definition.maxLevel >= minLevel and definition.minLevel <= maxLevel
end
