...
Tank Threat Control is an opt-in fixed-position HUD under Reminders → Threat Control. It shows a stable queue of up to five observed enemies, with a compact green health strip beneath every live enemy's full-height threat bar. While an enemy casts, that strip becomes an amber cast-progress meter; channels drain in reverse, protected casts use muted gray, and health returns when casting ends. A right-growing bar shows the player's remaining threat lead, while a left-growing bar shows the effort still needed to regain a lost mob. The threat bars represent relative distance from Blizzard's pull threshold, not seconds or a prediction of required attacks. Each live row also shows up to six player-applied debuffs beyond the current-target edge, with prominent centered stack counts, a fading pulse during the final five seconds, and compact overflow. Up to four rank-aware class anchors reserve consistent columns across mobs—Sunder Armor always occupies the Warrior's first slot—and missing anchors leave intentional gaps; other effects fill the remaining columns by spell ID. Rows keep their positions while a pack remains observed; if a hidden mob is lost, it replaces the safest visible held mob, and additional observations are reported with `+N`. During gameplay the HUD combines the current target, focus, mouseover, party and pet targets, and visible hostile nameplate units into one deduplicated pack view. Enemy nameplates provide the broadest coverage; without them, the HUD continues using target chains and labels its coverage as limited. The HUD never targets or casts. This view currently measures whether the player is tanking each enemy; it does not infer another party member as the tank.
...
When the current target is a living hostile mob, the movable Target HUD places a compact green player-health wing with absorb-shield and incoming-heal overlays to the left of a clear center gap and an equally prominent active-power wing to the right. Mana-only characters use mana as the right wing; classes and forms with both mana and another active resource keep mana as a slim secondary strip beneath the active resource. Missing or expiring Target Effects dock immediately left of health, with the highest-priority reminder nearest the bar and additional icons growing right to left. The shared display is anchored only to `UIParent`, remains available when enemy nameplates are disabled, and hides when the target is missing, friendly, or dead.
