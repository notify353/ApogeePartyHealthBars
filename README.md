...
Dungeon → Dungeon Guide opens a movable, opaque, read-only Dungeon Book; `/aphb guide` opens it directly. Its first strategy pack covers Scarlet Monastery Graveyard, Library, Armory, and Cathedral, including cleanup trash, bosses, and Graveyard rares. Each entry uses a compact standard: marker and name, Why, Plan, one combined Watch/CC line, and an If line only when an exception matters. Wing-specific pack and encounter rules follow the entries. The selected chapter is session-only; only the Book position is stored in the active profile. The prototype strategy remains subject to in-game review on both supported clients.

Automatic Dungeon Guide marking is on by default and can be disabled under Dungeon → Dungeon Guide. When the target changes, a cataloged living hostile receives its recommended Skull, Cross, or Moon out of combat; during combat, only Skull is applied. Any marker already on the current target is preserved, No Mark entries are ignored, and visible nameplates are not required. The controller is deliberately stateless: it never targets, scans a pack, assigns party members, clears markers, or casts. Because WoW does not expose a global marker owner, applying an icon can move that icon from an off-target enemy.
