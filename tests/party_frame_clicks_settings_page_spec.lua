...
function ApogeePartyHealthBars_ActionSettingsComponents.CreateActionRow(_, _, options)
    local row = widget()
    row.options = options
...
function ApogeePartyHealthBars_ActionSettingsComponents.LayoutActionList(list, rows)
...
dofile("Settings/PartyFrameClicksSettingsPage.lua")
local config = ApogeePartyHealthBars_PartyFrameClicksSettingsPage
...
assert(not valid and tostring(validationError):find("ClearBinding", 1, true),
    "PartyFrameClicksSettingsPage accepted incomplete dependencies")
...
assert(droppedFeature == "partyFrameClicks" and droppedSlot == "RightButton",
