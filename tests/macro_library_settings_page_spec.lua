...
function ApogeePartyHealthBars_ActionSettingsComponents.OpenViewer(options)
    viewedMacro = options
    return true
end
...
dofile("Settings/MacroLibrarySettingsPage.lua")
local config = ApogeePartyHealthBars_MacroLibrarySettingsPage
config.Create(widget(), { ApplyBackdrop = function() end })
