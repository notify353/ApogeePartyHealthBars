...
local function createSpellRow(parent, index)
    ...
    local upPriority = UIH.CreateButton(row, "^", 34, 22)
    upPriority:SetPoint("LEFT", label, "RIGHT", 3, 0)
    local downPriority = UIH.CreateButton(row, "v", 34, 22)
...
function DC.Refresh()
    ...
    for index, entry in ipairs(known) do
        ...
        row.reset.label:SetText(D.DotTracker.HasThresholdOverride(row.key) and "R*" or "R")
