...
assert(tocLoadOrder["Actions/BoundActionLayouts.lua"]
    ...
    and tocLoadOrder["Actions/BoundActionBindings.lua"]
        < tocLoadOrder["Actions/BoundActionRuntime.lua"]
    and tocLoadOrder["Actions/BoundActionDragController.lua"]
        < tocLoadOrder["Actions/BoundActionRuntime.lua"]
...
local guideMarkerLegend = ApogeePartyHealthBars_DungeonGuideUI.GetViewControls()
assert(guideMarkerLegend:IsShown() and guideScroll:IsShown()
        and ApogeePartyHealthBarsDungeonGuide.body:GetText():find("Scarlet Monastery — Graveyard", 1, true),
    "Dungeon Book did not open the initial Graveyard strategy chapter")
...
assert(guideScroll:GetVerticalScroll() == 0 and guideScroll:IsShown()
        and ApogeePartyHealthBarsDungeonGuide.body:GetText():find("Scarlet Monastery — Cathedral", 1, true),
    "Cathedral did not open its strategy chapter")
guideScroll:SetVerticalScroll(240)
...
assert(guideScroll:GetVerticalScroll() == 0
        and ApogeePartyHealthBarsDungeonGuide.body:GetText():find("Scarlet Monastery — Armory", 1, true),
    "changing to Armory did not load its strategy chapter")
...
assert(ApogeePartyHealthBars_DungeonGuideUI.IsShown()
    ...
    "Alt-left-click did not open the detected Uldaman guide")
...
assert(guideDungeonDropdown.selectedKey == "scarletMonastery"
    ...
    "Scarlet Monastery detection did not restore its last selected chapter")
...
assert(ApogeePartyHealthBars_ThreatAwareness == nil and ApogeePartyHealthBars_TargetEffectHud == nil and ApogeePartyHealthBars_CooldownHud == nil, "removed HUD modules still load")
assert(ApogeePartyHealthBars_SettingsUI.pages.threatControl == nil, "removed settings page remains")

assert(tocLoadOrder["PartyFrames/AccessoryLayout.lua"] < tocLoadOrder["PartyFrames/PlayerUtility.lua"]
    and tocLoadOrder["PartyFrames/AccessoryLayout.lua"] < tocLoadOrder["Actions/ShortcutBar.lua"],
    "retained accessories loaded before their layout dependency")
assert(type(ApogeePartyHealthBars_Threat.Refresh) == "function"
    and type(ApogeePartyHealthBars_ThreatObserver.GetThreatDetails) == "function",
    "party-frame threat support was removed with the tank HUD")
assert(ApogeePartyHealthBars_ThreatObserver.Refresh == nil,
    "multi-enemy observation runtime survived removal")
