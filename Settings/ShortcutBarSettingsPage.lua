...
ApogeePartyHealthBars_ShortcutBarSettingsPage = {}
...
local function openMacroEditor(slot)
    ...
    AC.OpenEditor({
        ...
        prefixBytes = ApogeePartyHealthBars_ActionMacros.GetWeaponPrefixBytes
            and ApogeePartyHealthBars_ActionMacros.GetWeaponPrefixBytes(entry) or 0,
        ...
        if visible then
            ...
            if entry then
                ...
                AC.SetActionRowState(row, {
                    ...
                    weaponSetName = shortcuts.GetSlotWeaponSet
                        and shortcuts.GetSlotWeaponSet(index),
...
function SC.Create(parent, deps)
    D = deps
    local shortcuts = D.ShortcutBar
    ...
    list = AC.CreateActionList(page, "ApogeePartyHealthBarsShortcutBarSettingsPageScroll")
    ...
    for index = 1, C.SHORTCUT_MAX_SLOTS do
        ...
        if row.weapon then
            row.weapon:SetSelectionCallback(function(setName)
                local ok, message = shortcuts.SetSlotWeaponSet(slot, setName)
