...
- Optional multi-enemy Threat Awareness HUD with worst-enemy, loss-only, and ranked-queue modes
...
- Passive target-nameplate reminders for usable, missing, or expiring player-applied DoTs and core maintained debuffs
...
Threat Awareness is an opt-in movable HUD under Frames → Party Frames. While configuring it, the Presentation selector previews Pack Radar, Loss Alarm, or Threat Queue with simulated enemies even while the live feature is disabled. Pack Radar shows the most urgent observed enemy, Loss Alarm stays hidden until an observed enemy is untanked, and Threat Queue ranks the five most urgent enemies and reports additional observations with `+N`. During gameplay it combines the current target, focus, mouseover, party and pet targets, and visible hostile nameplate units into one deduplicated pack view. Enemy nameplates provide the broadest coverage; without them, the HUD continues using target chains and labels its coverage as limited. Its optional sound plays only when a continuously observed enemy changes from tanked to lost, and the HUD never targets or casts.
...
When the current target is a living hostile mob with a visible enemy nameplate, large Moon, Cross, and Skull controls appear centered above that nameplate, with Skull on the right. Click a marker to apply or replace it, or click the current marker to remove it. After one of these markers is selected, the other two controls fade strongly while Blizzard's nameplate icon shows the active marker. When Target Effects are due, their smaller passive icons form a second centered row immediately above the marker controls; either row closes the gap automatically when the other is absent. The shared display remains hidden when the target has no visible nameplate, including when enemy nameplates are disabled or the mob is out of nameplate range.

Settings retain their compact 480×460 footprint so the Blizzard Spellbook, Settings, and live party-frame preview remain visible together. The party frames stay visible without a large configuration backing, while Cleanse Watch, Target Effects, and LFG Alert samples appear only on their relevant page. Five task groups—Frames, Actions, Reminders, Dungeon, and Manage—replace the crowded feature-tab row. Groups with multiple pages use a compact page selector for focused workflows such as Party Frame Clicks, Shortcut Bar, Keyboard, Mouse Wheel, Mouse Buttons, Health & Chat, Buffs & Cleansing, Target Effects, Profiles, Loadouts, Macro Library, and Maintenance; single-page groups show a simple page heading instead.

Target Effects controls a passive, click-through reminder row above the current hostile target's visible nameplate. It discovers learned aura-based damage effects and core maintained debuffs for the current class and client, shows only currently usable effects that are missing or inside their configured reminder timing, and never targets or casts. Damaging effects require the player's own aura; an equal or stronger equivalent maintained debuff from another player counts as covered. Reminders remain tracked but hidden while the target has no visible nameplate. Reminders → Target Effects provides an inline sample plus profile-owned spell order, enablement, and global or per-spell timing; placement is automatic and is not saved. Profiles distinguishes the selected profile from the active profile and groups creation, replacement, and sharing into compact sections. Each character owns an independent profile library; profiles are never changed or selected implicitly by another character. A profile contains all feature settings, action assignments, custom macros, sounds, and movable positions. Use Create for addon defaults, Duplicate to branch from an existing setup, or Replace Active to overwrite the active setup while retaining its name. Profile changes are blocked in combat and reload the UI after safely restoring owned Keyboard, Mouse Wheel, and Mouse Buttons bindings.
...
While add-on settings are open, drag a Spellbook spell or an item from an open bag directly onto a Party Frame Clicks, Shortcut Bar, Keyboard, Mouse Wheel, or Mouse Buttons HUD position, or onto its row in settings. HUD drops are disabled when settings are closed so normal bag interaction cannot change an assignment. Spell drops use the highest learned rank by default; hold Shift while dropping to preserve the selected Spellbook rank. For bag items, clicking the item and then its settings destination works too. Shortcut Bar, Keyboard, Mouse Wheel, and Mouse Buttons use the same compact action rows; settings exposes one extra empty Shortcut Bar row for adding the next action. Drop directly onto an occupied row to replace it. Shortcut Bar supports up to 12 assignments and displays them six per row in a footer beneath the complete party-health frame. Shortcut Bar rejects duplicate spell and item IDs; Keyboard, Mouse Wheel, and Mouse Buttons permit the same spell or item in multiple positions and across features. Action changes are blocked in combat.
...
Manage → Macro Library is the in-addon reference for generated templates, macro syntax, and curated combat recipes. Each compact topic explains where Apogee applies the pattern, why it is useful, and when custom text may be preferable; its Macro button opens the exact body in a focused read-only viewer. Executable templates and current-class recipes remain selectable for copying into WoW's Macro window, while syntax-only examples are clearly marked as reference material. The library also covers optional channel guards, forced Hunter Auto Shot, mouseover and focus targeting, `/stopattack`, cursor casting, help/harm and modifier choices, stealth protection, and queued next-swing attacks. It never creates, updates, or tracks game macros.
