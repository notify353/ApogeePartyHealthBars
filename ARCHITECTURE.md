...
- `TargetNameplateHud`: current-target GUID/nameplate matching and reusable ordered attachment for passive player-status and Target Effects surfaces, with one-way anchoring from a permanently `UIParent`-owned root that never enters or mutates Blizzard's nameplate hierarchy
