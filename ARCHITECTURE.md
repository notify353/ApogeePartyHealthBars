...
- `GroupHelperEvents`: dungeon-session, roster, combat, aura, connection, health, and mana observation routing with debounced out-of-combat refreshes
...
- `WeaponSets`: native character-wide Main Hand and Off Hand set capability, compatibility filtering, deferred post-save verification, explicit incompatible-set conversion, name-based action attachment, secure prefix composition, and combined runtime byte validation
- `ActionMacros`: shared action-intent template rendering and documentation metadata, direct `/use` spell/item defaults, player-feet explosive defaults, localized curated melee, distance, and stealth-safe families, dedicated repeating ranged-attack quality-of-life behavior, sound/macro extensions, custom-text detection, weapon-set prefix composition, and 255-byte validation for Shortcuts, Keyboard, Mouse Wheel, and Mouse Buttons
- `ActionSettingsComponents`: shared scrollable action-list scaffold and compact row state used by Healing, Shortcuts, Keyboard, Mouse Wheel, and Mouse Buttons, plus the weapon-set picker and focused macro editor used by the macro-capable features
...
- `UnitAPI`: narrow compatibility boundary for unit existence, identity, health, casting, connection, range, healability, and adaptive power channels
- `UnitTopology`: fixed player/party and immediate-target token graph plus token-to-owner resolution
- `UnitBar`: shared 36px health rows with class identity rails, adaptive power, shield, incoming-heal, HoT, range/offline, party-buff, secure Party-Frame-Click surfaces, and an explicit synthetic preview-model boundary
...
- `Layout`: stable three-column row positioning, left-expanding Group Helper/threat companion reservation, and secure overlay placement using `RowGeometry`
...
- `GroupHelperData`, `GroupHelperPolicy`: Classic-first drink-aura and long-duration group-buff catalogs plus pure dungeon eligibility, aura-first drinking visibility, sub-75% thirsty policy, provider-aware Fortitude/Intellect/Mark/blessing coverage, evidence-gated Divine Spirit coverage, and an independently eligible solo/outdoor pull call
- `GroupHelperRuntime`, `GroupHelperSettings`: GUID-keyed roster and observed-provider-capability state, immutable drink/buff status snapshots, hardware-click-only direct `/say` Group Calls, and portable profile-owned enablement
- `GroupHelperPresentation`, `PartyFramePreview`: compact clickable GUID-to-party-row drink chips and provider-aligned missing-buff icons with locally ticking observed-aura countdowns, a single independently visible solo/outdoor pull icon above the rows, hardware-click direct `/say` behavior in preview, combat-safe companion reservation, and session-only scenarios that drive the real five-player panel without impersonating WoW unit APIs or activating secure frame actions
- `Threat`: combat-only per-row threat rail/current-target margin rendering plus an explicit synthetic preview snapshot; live unit APIs are never consulted while preview data owns the surface
...
- `TargetEffectData`, `TargetEffectTracker`, `TargetEffectHud`: ID-only learned-rank catalog; player-owned harmful-aura evaluation; event/timer-driven target suggestions; and passive right-to-left reminders on Threat Control's left
- `ThreatControlSettingsPage`: consolidated Threat Control, maintained-effect, and Ability Cooldowns configuration
- `CooldownData`, `CooldownTracker`, `CooldownHud`: reviewed class/pet family catalog; portable default/override/priority policy; event-authoritative cooldown, usability, and charge state; and a passive fixed-slot lane on Threat Control's right
...
- Keep player health and active power inside the fixed `ThreatAwareness` topology. `TargetEffectHud` owns only the passive right-to-left reminder row immediately left of those bars; `CooldownHud` owns only the passive, mouse-disabled, six-slot cooldown row immediately right. Both match the enemy debuff-lane geometry, and neither may shift the player bars or enemy rows. Missing optional aura, Spellbook, cooldown, charge, or usability APIs disable only the affected lane without rewriting profile intent or hiding Threat Control.
...
- Treat Tank Threat Control as an observable-token view, never a complete combat-enemy list. Exclude the locale-independent Totem creature type before threat work and coverage accounting, deduplicate other hostile sources by GUID, express held lead and lost recovery as one signed player-control value, label reduced coverage when no eligible hostile nameplate tokens are observable, show only player-owned harmful auras available through the selected live token, and never present cached threat or debuff values as live after the final token disappears.
- Keep Tank Threat Control passive and non-secure. Preserve stable row slots until the observed pack empties, promoting a hidden live loss over a non-live last-seen warning first and then the safest visible held mob without otherwise reordering.
- Keep the enabled Threat Control frame visible outside combat and with an empty observed pack so player status and selected cooldowns remain persistent; only enemy rows and their footer collapse when no enemies are observed. Do not run full hostile-token/threat scans outside combat merely to preserve that shell.
- Poll immediate-target identity and values at the normal visual cadence because target-chain event delivery is not reliable enough to own live row geometry.
...
- Pre-create all primary and immediate-target surfaces plus secure Party Frame Click overlays before combat; missing targets hide their surfaces without collapsing the reserved target column.
...
- Keep native weapon sets character-wide and outside profiles. Persist only an optional compatible set name on macro-capable actions, preserve unavailable names, and never export native set IDs or contents.
- Accept only native sets that ignore every slot except Main Hand and Off Hand. Compose one unconditional secure `/equipset` prefix; if the set is missing or externally broadened, run the saved action unchanged. Party Frame Clicks must remain unchanged.
- Apply the existing 255-byte policy to the complete weapon-set prefix and action-body macro before attachment or editing.
...
- Keep reviewed class/pet cooldown identity, category, default enablement, and default priority inside `CooldownData`. `CooldownTracker` may observe only learned Spellbook families through compatibility adapters, must ignore the global cooldown, and must preserve explicit overrides plus a maximum of six ordered selections. `CooldownHud` remains non-secure, cast-free, mouse-disabled, quiet, and position-stable.
...
- Never call Blizzard Spellbook toggles, replace Spellbook or bag-item scripts, or hook their click handlers; use source visibility and destination-based cursor drops.
