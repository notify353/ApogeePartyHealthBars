...
ApogeePartyHealthBars_MacroLibrarySettingsPage = {}
local M = ApogeePartyHealthBars_MacroLibrarySettingsPage
local page, D, form, categoryDropdown, title, description, applied, whyText, tradeoffs, recipeState
local statusText
...
function M.Create(parent, deps)
    D = deps
    page = CreateFrame("Frame", nil, parent)
    page:SetPoint("TOPLEFT", parent, "TOPLEFT", C.BIND_PAD, -(C.CONFIG_HEADER_H + C.BIND_PAD + C.CONFIG_PAGE_SELECTOR_H + 4))
    page:SetPoint("BOTTOMRIGHT", parent, "BOTTOMRIGHT", -C.BIND_PAD, C.BIND_PAD)
    page:Hide()

    local _, className = playerClass()
    form = UIH.CreateFormScaffold(page, "ApogeePartyHealthBarsMacroLibrarySettingsPageScroll",
