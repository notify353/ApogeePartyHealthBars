...
Dungeon → Dungeon Guide opens a movable, opaque, read-only Dungeon Book; `/aphb guide` opens it directly. Its strategy packs cover Scarlet Monastery and Gnomeregan. Scarlet Monastery includes Graveyard, Library, Armory, and Cathedral trash, bosses, rares, and encounter rules. Gnomeregan follows the full front-entrance route through the Hall of Gears and Trogg Caves, Dormitory and Launch Bay, Engineering Labs, and Tinkers' Court, with backtracking, Workshop Key shortcut, alarm, mine, bomb, ledge, and boss guidance. Each entry uses a compact standard: marker and name, Why, Plan, one combined Watch/CC line, and an If line only when an exception matters. Route guidance appears before the entries and pack or encounter rules follow them. The selected chapter is session-only; only the Book position is stored in the active profile.

Automatic Dungeon Guide marking is on by default and can be disabled under Dungeon → Dungeon Guide. When the target changes, a cataloged living hostile receives its recommended Skull, Cross, Moon, or boss Circle out of combat; during combat, only Skull and boss Circle are applied. Any marker already on the current target is preserved, No Mark entries are ignored, and visible nameplates are not required. The controller is deliberately stateless: it never targets, scans a pack, assigns party members, clears markers, or casts. Because WoW does not expose a global marker owner, applying an icon can move that icon from an off-target enemy.

Settings retain their compact 480×460 footprint so the Blizzard Spellbook, Settings, and live party-frame preview remain visible together. The party frames stay visible without a large configuration backing, while Cleanse Watch, Target Effects, and LFG Alert samples appear only on their relevant page. Five task groups—Frames, Actions, Reminders, Dungeon, and Manage—replace the crowded feature-tab row. Groups with multiple pages use a compact page selector for focused workflows such as Party Frame Clicks, Shortcut Bar, Keyboard, Mouse Wheel, Mouse Buttons, Health & Chat, Buffs & Cleansing, Target Effects, Dungeon Board, Dungeon Guide, Profiles, Loadouts, and Maintenance; single-page groups show a simple page heading instead.
...
Target Effects controls a passive, click-through reminder row above the current hostile target's visible nameplate. It discovers learned aura-based damage effects and core maintained debuffs for the current class and client, shows only currently usable effects that are missing or inside the shared reminder timing, and never targets or casts. Damaging effects require the player's own aura; an equal or stronger equivalent maintained debuff from another player counts as covered. Reminders remain tracked but hidden while the target has no visible nameplate. Reminders → Target Effects provides an inline sample plus one profile-owned reminder timer, spell order, and enablement; placement is automatic and is not saved. Profiles distinguishes the selected profile from the active profile and groups creation, replacement, and sharing into compact sections. Each character owns an independent profile library; profiles are never changed or selected implicitly by another character. A profile contains all feature settings, action assignments, custom macros, sounds, and movable positions. Use Create for addon defaults, Duplicate to branch from an existing setup, or Replace Active to overwrite the active setup while retaining its name. Profile changes are blocked in combat and reload the UI after safely restoring owned Keyboard, Mouse Wheel, and Mouse Buttons bindings.
