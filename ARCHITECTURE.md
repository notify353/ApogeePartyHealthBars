...
- `RuntimeDungeonBoardEvents`: authoritative `CHAT_MSG_CHANNEL` payload adaptation, direct event subscription, duplicate-safe chat-filter fallback, and login initialization for Dungeon Board collection
...
- Keep Dungeon Board catalog and classification independent from chat events, saved variables, UI, localization, and Blizzard Group Finder APIs; the session-only runtime consumes classifier results, the event adapter alone owns chat payload knowledge and duplicate-safe delivery paths, and the UI consumes only immutable runtime snapshots.
