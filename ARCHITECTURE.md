...
- `PlayerStatusHud`, `TargetEffectData`, `TargetEffectTracker`, `TargetEffectHud`, `TargetEffectsSettingsPage`: event-driven split player-health/active-power wings with shield, incoming-heal, and optional secondary-mana presentation; ID-only learned-rank catalog; player-owned harmful-aura evaluation; event/timer-driven target suggestions; left-growing passive reminders; combined inline configuration sample; and profile-owned enablement/priority/threshold controls
...
- `TargetNameplateHud`: legacy-named coordinator for current-target GUID eligibility, a stable profile-owned center-gap anchor, calculated combined drag bounds, and role-based split-base/left-accessory placement beneath a permanently `UIParent`-owned root
...
- `ThreatAwareness`: passive fixed-position five-slot Tank Threat Control presentation, stable per-pull row reconciliation, rank-aware class debuff columns with deterministic fallback, final-five-second pulses and centered stacks, overflow loss promotion, noninteractive configuration preview
...
- Keep hostile-target eligibility and shared placement inside `TargetNameplateHud`; despite its legacy name, it must never acquire, inspect, parent to, anchor to, or otherwise retain a Blizzard nameplate frame. Its saved root marks the split HUD's center gap, while Apogee-owned drag bounds may resize around left-growing reminders without moving that anchor. `PlayerStatusHud` owns the split health/power base, Target Effects owns its left accessory row, and absent reminders collapse without shifting either wing. Missing harmful-aura, Spellbook, or action-state APIs disable Target Effects without hiding health and power.
