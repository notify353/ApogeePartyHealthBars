...
- `UnitFrames`: row composition, stable secure frame creation, and external feature attachment
- `RowGeometry`: shared adaptive unit chrome, player action-area height, and tallest-surface row height
...
- `PlayerUtility`: player-only self-buff icon, accessory inset, and secure binding, attached through the player unit-bar interface
...
- `ShortcutBar`, `ShortcutConfig`: 12-slot typed shortcut storage, independently anchored player/crowd-control grids, spell/item state icons, sound feedback, secure macros, smart Spellbook/bag assignment, and scrollable compact configuration
...
- Poll target-chain GUIDs at the normal visual cadence because Anniversary's Blizzard raid frames document unreliable second-depth target events.
